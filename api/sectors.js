...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Helper: fetch with timeout to avoid Vercel's 10s function ceiling
    const fetchWithTimeout = async (url, opts = {}, ms = 3500) => {
    ...
    const fetchQuoteFinnhub = async (sym) => {
      try {
        ...
        failureReasons.push(e.message === 'timeout' ? 'finnhub_timeout' : 'finnhub_exception');
        return null;
      }
    };
    ...
    const fetchQuoteYahoo = async (sym) => {
      try {
        ...
        const r = await fetchWithTimeout(url, { headers: { 'User-Agent': 'Mozilla/5.0' } }, 3500);
        if (!r.ok) {
        ...
        failureReasons.push(e.message === 'timeout' ? 'yahoo_timeout' : 'yahoo_exception');
        return null;
      }
    };

    // Universal resilient fetcher: try Finnhub first, then Yahoo automatically.
    // Wrapped in its own try-catch so one bad sector NEVER crashes the endpoint.
      ...
      try {
        ...
        if (sectorDef.altTickers) {
          for (const alt of sectorDef.altTickers) {
            if (alt === sectorDef.ticker) continue;
        ...
        // Defensive: any unexpected error in resilient fetcher returns null,
        // so a single bad ticker doesn't poison the whole batch
        failureReasons.push('resilient_exception');
        return null;
      }
    };

    // Use Promise.allSettled — if ONE promise rejects, others still resolve
    const sectorPromises = sectorList.map(s =>
      fetchQuoteResilient(s).then(result => {
      ...
      }).catch(() => null)  // any unhandled error becomes null, doesn't crash
    );

    const refPromises = REFS.map(r =>
      fetchQuoteResilient(r).then(result => {
        if (!result) return null;
        return { ticker: r.ticker, name: r.name, ...result.quote };
      }).catch(() => null)
    );

    const [sectorsSettled, refsSettled] = await Promise.all([
      Promise.allSettled(sectorPromises),
      Promise.allSettled(refPromises)
    ]);

    const sectorsResult = sectorsSettled.map(p => p.status === 'fulfilled' ? p.value : null);
    const refsResult = refsSettled.map(p => p.status === 'fulfilled' ? p.value : null);
    ...
    if (sectors.length === 0) {
      ...
      } else if (reasons.includes('finnhub_timeout') || reasons.includes('yahoo_timeout')) {
        errorMsg = 'Data sources slow today. Try again in 30 seconds.';
      } else if (reasons.length === 0) {
        errorMsg = 'No quotes returned (market may be closed or tickers unrecognized).';
      } else {
        errorMsg = 'Data sources failing: ' + reasons.slice(0, 3).join(', ');
      }
    }
