...
/** Las grafías candidatas del rango en /v2/historicos. */
function candidatosHistoricos(desde, hasta) {
  return [
    { etiqueta: 'inicio/final', inicio: 'inicio', final: 'final', params: { inicio: desde, final: hasta } },
    { etiqueta: 'inicio/fin', inicio: 'inicio', final: 'fin', params: { inicio: desde, fin: hasta } },
    { etiqueta: 'fecha_inicio/fecha_final', inicio: 'fecha_inicio', final: 'fecha_final', params: { fecha_inicio: desde, fecha_final: hasta } },
    { etiqueta: 'desde/hasta', inicio: 'desde', final: 'hasta', params: { desde, hasta } },
    { etiqueta: 'start/end', inicio: 'start', final: 'end', params: { start: desde, end: hasta } },
  ];
}
...
function paramsFinancieros(contrato, emisora, anio, trimestre) {
  ...
  const valor = c.forma === 'cierre' ? finDeTrimestre(anio, trimestre) : clavePeriodo(anio, trimestre);
  return { ...base, [c.periodo || 'periodo']: valor };
}

function paramsHistoricos(contrato, emisora, desde, hasta) {
  const c = (contrato && contrato.historicos) || CONTRATO_DEFECTO.historicos;
  return { emisora, [c.inicio || 'inicio']: desde, [c.final || 'final']: hasta };
}
...
function estimarConsumo({ emisoras, camposPorFinanciero = 60, preciosDesde = null } = {}) {
  ...
  for (const e of emisoras) {
    ...
    const trimestres = rango ? trimestresEntre(rango.desde, rango.hasta).length : 0;
    reqFin += trimestres;
    ...
    det.push({ emisora: e.emisora, trimestres, dias_habiles_estimados: dias });
  }
...
async function jobProbe(req) {
  ...
  for (const cand of candidatosHistoricos(desde, hasta)) {
    if (!cartera.puedeSeguir()) break;
    const r = await traer(construirUrl('/historicos', { emisora, ...cand.params }));
    cartera.anota(r);
    const plano = r.ok ? aplanarHistoricos(r.json) : { filas: [], descartadas: 0 };
    pasos.push({
      paso: 'historicos', candidato: cand.etiqueta, url: r.url, status: r.status,
    ...
    if (plano.filas.length) {
      contrato.historicos = { inicio: cand.inicio, final: cand.final };
      break;
    }
    await dormir(PAUSA_MS);
  }
...
/** Normaliza una fila del censo sin importar cómo venga envuelta. */
function filaCenso(clave, valor) {
...
async function jobHistoricos(req) {
  ...
  const objetivo = [...ics];
  if (!objetivo.some((e) => e.emisora === BENCHMARK)) {
    const b = await emisoraPorClave(BENCHMARK);
    objetivo.push(b || { emisora: BENCHMARK, hist_desde: null, hist_hasta: null });
  }
  ...
  for (const e of objetivo) {
    ...
    const tengoHasta = yaTengo.get(e.emisora);
    ...
    if (porAnio) {
      for (let a = Number(d.slice(0, 4)); a <= Number(h.slice(0, 4)); a++) {
        ...
        const clave = `${di}..${hf}`;
        if (hechas.has(`${e.emisora}|${clave}`)) continue;
        pendientes.push({ emisora: e.emisora, desde: di, hasta: hf, clave });
      }
    } else {
      const clave = `${d}..${h}`;
      if (hechas.has(`${e.emisora}|${clave}`)) continue;
      pendientes.push({ emisora: e.emisora, desde: d, hasta: h, clave });
    }
  }
  ...
  for (; i < pendientes.length; i++) {
    ...
    if (!r.ok) {
      await marcarLedger('historicos', p.emisora, p.clave, { estado: 'error', requests: 1, error_msg: `${r.status}: ${r.error}` });
      hecho.push({ ...p, estado: 'error', error: r.error });
    } else {
      const { filas, descartadas } = aplanarHistoricos(r.json);
      const escritas = filas.length ? await insertarPrecios(p.emisora, filas) : 0;
      const conImporte = filas.filter((f) => f.importe !== null).length;
      await marcarLedger('historicos', p.emisora, p.clave, {
...
export default async function handler(req, res) {
  ...
  const protegidos = new Set(['probe', 'emisoras', 'financieros', 'historicos']);
